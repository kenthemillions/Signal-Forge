...
return `<button type="button" class="btn btn-sm ${btnClass} last-hour-play" data-symbol="${p.symbol}" data-play="${p.play}" title="${safeReason}">${p.symbol} strong ${p.play} <span class="badge bg-dark">${p.strength_score}</span></button>`;
     ...
     * If overrideSymbol is passed, use it for fetch and set currentTicker so quote card is independent of other modules.
     */
    async loadTickerCardQuote(overrideSymbol) {
        const sym = (overrideSymbol != null && overrideSymbol !== '') ? String(overrideSymbol).trim().toUpperCase() : (this.currentTicker || 'SPY').trim().toUpperCase();
        this.currentTicker = sym;
        var select = document.getElementById('ticker-select');
        if (select) {
            var hasOpt = Array.from(select.options).some(function(o) { return o.value === sym; });
            if (!hasOpt) {
        ...
        this._updateQuoteDebug({ loadTickerCardQuoteCalled: true, lastTouch: 'loadTickerCardQuote (entry)', currentTickerAssigned: sym });
    ...
    _setAddDebug(updates) {
        var ids = ['add-debug-clicked', 'add-debug-raw', 'add-debug-normalized', 'add-debug-added-to-list', 'add-debug-select-value', 'add-debug-current-ticker', 'add-debug-quote-url', 'add-debug-quote-status', 'add-debug-quote-body', 'add-debug-dom-ok'];
        ids.forEach(function(id) {
            if (updates[id] !== undefined) {
                var el = document.getElementById(id);
                if (el) el.textContent = String(updates[id]);
            }
        });
    }
    async addTicker() {
        ...
        var symbol = raw.replace(/,/g, ' ').split(/\s+/).map(function(s) { return s.trim().toUpperCase(); }).filter(Boolean)[0] || raw.trim().toUpperCase();
        self._setAddDebug({ 'add-debug-clicked': 'yes', 'add-debug-raw': raw, 'add-debug-normalized': symbol });

        self.currentTicker = symbol;
        var select = document.getElementById('ticker-select');
        if (select) {
            var exists = Array.from(select.options).some(function(opt) { return opt.value === symbol; });
            if (!exists) {
                var opt = document.createElement('option');
                opt.value = symbol;
            ...
            select.value = symbol;
            self._setAddDebug({ 'add-debug-added-to-list': exists ? 'already' : 'yes', 'add-debug-select-value': select.value });
        }
        self._setAddDebug({ 'add-debug-current-ticker': self.currentTicker, 'add-debug-quote-url': '/api/quote?symbol=' + encodeURIComponent(symbol) });

        var submitBtn = document.getElementById('add-ticker-submit');
        if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = 'Adding...'; }

        await self.loadTickerCardQuote(symbol);
        if (self._quoteDebug) {
            self._setAddDebug({
                'add-debug-quote-status': self._quoteDebug.quoteStatus !== undefined ? String(self._quoteDebug.quoteStatus) : '--',
                'add-debug-quote-body': (self._quoteDebug.quoteBody && self._quoteDebug.quoteBody.length > 80) ? self._quoteDebug.quoteBody.substring(0, 80) + '...' : (self._quoteDebug.quoteBody || '--'),
                'add-debug-dom-ok': self._quoteDebug.domUpdateSuccess ? 'yes' : 'no'
            });
        }
        ...
        try {
            ...
            if (res.ok) {
                var added = data.added || (data.symbol ? [data] : []);
                var errors = data.errors || [];
                added.forEach(function(t) { self.scannerTickerSelection[t.symbol] = true; });
                self.saveScannerSelection();
                self.loadTickers().then(function() {
                    var sel = document.getElementById('ticker-select');
                    if (sel && symbol) {
                        var has = Array.from(sel.options).some(function(o) { return o.value === symbol; });
                        if (!has) {
                ...
                if (added.length) self.showTickerToast('Added: ' + added.map(function(t) { return t.symbol; }).join(', '), 'success');
                if (errors.length) self.showTickerToast(errors.slice(0, 3).join('; '), 'warning');
            } else {
                self.showTickerToast((data.error || 'Could not save to list') + '; quote loaded.', 'warning');
            }
            var modal = bootstrap.Modal.getInstance(document.getElementById('addTickerModal'));
            if (modal) modal.hide();
        } catch (error) {
            console.error('Error adding ticker:', error);
            self.showTickerToast('List save failed; quote for ' + symbol + ' loaded.', 'warning');
            var modal = bootstrap.Modal.getInstance(document.getElementById('addTickerModal'));
            if (modal) modal.hide();
        } finally {
