class TradingSignalsApp {
  ...
  loadPerformanceData() {
    try {
      ...
      return stored
        ? JSON.parse(stored)
        : { signals: [], wins: 0, losses: 0, totalGain: 0, totalLoss: 0 };
    } catch (e) {
  ...
  trackSignalPerformance(data) {
    ...
    if (data.main_signal && data.main_signal !== "WAIT") {
      const hasRecent = signals.some(
        (s) =>
          s.ticker === this.currentTicker && Date.now() - s.timestamp < 300000,
      );
      ...
      if (!hasRecent) {
        ...
        this.performanceData.totalSignals =
          (this.performanceData.totalSignals || 0) + 1;
      }
    }
  ...
  updatePerformanceDisplay() {
    ...
    const winRate = hasEnoughData
      ? ((this.performanceData.wins / resolved) * 100).toFixed(0)
      : "--";
    const avgWin =
      this.performanceData.wins > 0
        ? (this.performanceData.totalGain / this.performanceData.wins).toFixed(
            1,
          )
        : "0";
    const avgLoss =
      this.performanceData.losses > 0
        ? (
            this.performanceData.totalLoss / this.performanceData.losses
          ).toFixed(1)
        : "0";
    ...
    if (winRateBadgeEl) {
      if (!hasEnoughData) {
        ...
        winRateBadgeEl.className =
          "badge " +
          (rate >= 60 ? "bg-success" : rate >= 50 ? "bg-info" : "bg-warning");
      }
    }
    ...
    const tfStats = {};
    (this.performanceData.signals || [])
      .filter((s) => s.resolved && s.outcome === "win")
      .forEach((s) => {
    ...
    if (bestTfEl)
      bestTfEl.textContent = hasEnoughData
        ? `${bestTf} - ${winRate}% win rate`
        : `${bestTf} - collecting data`;
  }
  ...
  startLoadingTimeout() {
    setTimeout(() => {
      ...
      if (
        textEl &&
        (textEl.textContent === "Loading market status..." ||
          textEl.textContent === "Connecting…")
      ) {
        textEl.textContent =
          "Server may be waking up. Click Refresh in a moment.";
        const iconEl = document.getElementById("session-icon");
        if (iconEl) iconEl.textContent = "📡";
      }
      if (
        premarketTrend &&
        premarketTrend.textContent.trim() === "Loading..."
      ) {
  ...
  async loadPremarketAnalysis() {
    try {
      const response = await fetch(
        `/api/premarket-analysis/${this.currentTicker}?_t=${Date.now()}`,
      );
        ...
        try {
          ...
          if (data.success) {
            this.showNotification(
              "Thank you for your feedback! We truly appreciate beta testers like you.",
              "success",
            );
            ...
            this.showNotification(
              data.error || "Error submitting feedback",
              "danger",
            );
          }
        } catch (error) {
  ...
  initAudioContext() {
    try {
      ...
      this.alertSound = new Audio(
        "https://assets.mixkit.co/active_storage/sfx/1337/1337-preview.mp3",
      );
  ...
  enableTradingAlerts() {
    ...
    if (this.bellSound)
      this.bellSound
        .play()
        .then(() => this.bellSound.pause())
        .catch(() => {});
    if (this.alertSound)
      this.alertSound
        .play()
        .then(() => this.alertSound.pause())
        .catch(() => {});
    ...
    this.showNotification(
      "Trading alerts enabled! You'll hear bells at 3:50 PM and 3:54 PM ET.",
    );
  ...
  showNotification(message, type = "info") {
    ...
    toast.style.cssText =
      "top: 70px; right: 20px; z-index: 9999; max-width: 350px; animation: fadeIn 0.3s;";
  ...
  showEndOfDayCheckIn() {
    ...
    const responses = {
      great: [
        ...
        "Excellent! Remember what worked today.",
      ],
      okay: [
        "Every day is a lesson. Tomorrow is a new opportunity.",
        "Steady progress beats big swings. You're doing fine.",
        "Review your trades. Small adjustments lead to big results.",
      ],
      tough: [
        "Tough days build tough traders. You've got this.",
        "Protect your capital. Live to trade another day.",
        "Step back, breathe. The market will be there tomorrow.",
      ],
    };
  ...
  checkLotteryHourAlerts() {
    ...
    if (hours === 15 && minutes === 50 && this.lastLotteryAlert !== alertKey) {
      ...
      this.showNotification(
        "🔔 LOTTERY HOUR in 5 minutes! High momentum trades only.",
        "warning",
      );
    }
    ...
    if (
      ...
      this.showNotification(
        "⚠️ 2 MINUTES to Lottery Hour! Auto-scanning now...",
        "danger",
      );
    ...
    if (
      ...
      this.lastLotteryAlert = alertKey55;
      this.showNotification(
        "🎰 LOTTERY HOUR IS NOW! Extreme momentum plays only!",
        "danger",
      );
    ...
    if (
      ...
      this.speak(
        `Good morning trader! The market opens in 5 minutes. Remember: ${quote}. Let's have a great trading day!`,
      );
    }
    ...
    if (
      ...
      this.playMarketOpenMelody();
      this.showNotification(
        "🔔 MARKET IS NOW OPEN! Time to execute!",
        "success",
      );
      this.speak(
        "The bell has rung! Market is open. Stay focused, stay disciplined, and trade your plan!",
      );
    }
    ...
    if (
      ...
      this.showNotification(
        "🎰 LOTTERY HOUR! High momentum plays only!",
        "danger",
      );
      this.speak(
        "Lottery hour is now active! Only high conviction trades. Protect your gains!",
      );
    }
    ...
    if (
      ...
      this.speak(
        "The closing bell has rung. Great work today! Take a moment to review your trades.",
      );
    ...
    try {
      ...
      if (!data.strongest_plays || data.strongest_plays.length === 0) {
        if (data.in_last_hour_window) {
          playsEl.innerHTML =
            '<span class="text-muted small">No strong plays in last hour yet.</span>';
      ...
      if (timeEl) timeEl.textContent = data.scan_time || "";
      playsEl.innerHTML = data.strongest_plays
        .map((p) => {
  ...
  handleTrendReversalAlert(data) {
    ...
    const key =
      (this.currentTicker || "") + (data.trend_reversal_direction || "");
    ...
    const label =
      dir === "BULLISH_TO_BEARISH"
        ? "⚠️ TREND REVERSAL — BULLISH TO BEARISH"
        : "⚠️ TREND REVERSAL — BEARISH TO BULLISH";
    if (textEl) textEl.textContent = label + " " + (this.currentTicker || "");
    if (reasonEl) {
      reasonEl.textContent =
        data.trend_reversal_reason ||
        "Structure break or multi-timeframe flip.";
    ...
    if (isNewReversal && this.audioEnabled) {
      ...
      if (this.sendPushNotification) {
        this.sendPushNotification(
          "TREND REVERSAL — " + this.currentTicker,
          data.trend_reversal_reason || "The trend is changing. Beware.",
          "reversal",
        );
      }
    }
  ...
  activateLotteryHourMode() {
    ...
    this.showNotification(
      "🎰 LOTTERY HOUR ACTIVE - Extreme moves possible!",
      "warning",
    );
  }
  ...
  async runLotteryHourScan() {
    ...
    this.showNotification(
      `🎰 LOTTERY SCAN: Analyzing ${allTickers.length} tickers...`,
      "warning",
    );
    ...
    for (const ticker of allTickers) {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          const isExtended = extendedHoursTickers.includes(
            ticker.toUpperCase(),
          );
      ...
      if (data.success && data.lottery_picks.length > 0) {
        const picksHtml = data.lottery_picks
          .map((pick, i) => {
            ...
            const changeDisplay =
              priceChange !== 0
                ? `<span class="${priceChange >= 0 ? "text-success" : "text-danger"}">${priceChange >= 0 ? "+" : ""}${priceChange.toFixed(2)}%</span>`
                : "";
            return `
        ...
        this.showNotification(
          `🎰 Found ${data.lottery_picks.length} lottery plays!`,
          "warning",
        );
      } else {
        content.innerHTML = `
  ...
  async runMarketOpenScan(phase = "5min") {
    ...
    try {
      ...
      if (data.success && data.trending_picks.length > 0) {
        const picksHtml = data.trending_picks
          .map((pick, i) => {
            const isCall = pick.option_type === "CALL";
        ...
        this.showNotification(
          `📈 Found ${data.trending_picks.length} trending stocks!`,
          "info",
        );
      } else {
        content.innerHTML = `
  ...
  checkMarketOpenAlerts() {
    ...
    if (
      ...
      this.showNotification(
        "🌅 Pre-market scan complete! Top movers identified.",
        "info",
      );
    }
    ...
    if (
      ...
      this.showNotification(
        "⏱️ 5-Minute scan! Initial trends forming...",
        "info",
      );
    }
    ...
    if (
      ...
      this.showNotification(
        "✅ 30-Minute scan! True trends identified!",
        "success",
      );
    }
  }
  ...
  initMarketOpenScanner() {
    document.querySelectorAll(".open-scan-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        document
          .querySelectorAll(".open-scan-btn")
          .forEach((b) => b.classList.remove("active"));
  ...
  initSocket() {
    ...
    this.socket.on("new_signal", (signal) => {
      ...
      if (this.audioEnabled && signal.entry_alert) {
        const isNewSignal =
          !this.lastSignal || this.lastSignal.type !== signal.signal_type;
        if (isNewSignal) {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) => {
                const vol = ctx.raw;
                if (vol >= 1000000)
                  return `Vol: ${(vol / 1000000).toFixed(1)}M`;
  ...
  updateVolumeChart(volumes, closes, opens) {
    ...
    if (spikeAlert) {
      if (
        spikeCount >= 3 ||
        (hasSpike && volumes[volumes.length - 1] / avgVolume >= 2)
      ) {
  ...
  addVolumeGlow() {
    ...
    if (container) {
      container.style.boxShadow =
        "0 0 20px rgba(255, 165, 0, 0.6), 0 0 40px rgba(255, 69, 0, 0.3)";
      ...
      options: {
        ...
        scales: {
          ...
          rsi: {
            ...
            max: 100,
            grid: { display: false },
            ticks: { color: "#22C55E", stepSize: 30 },
            title: { display: true, text: "RSI", color: "#22C55E" },
          },
        },
      },
    });
  ...
  createCandlestickChart() {
    ...
    const config =
      this.TIMEFRAME_CONFIG[this.currentInterval] ||
      this.TIMEFRAME_CONFIG["5m"];
  ...
  async loadTickers() {
    ...
    if (tickers.length > 0) {
      const hasCurrent = tickers.some(
        (t) => (t.symbol || t) === this.currentTicker,
      );
      if (!hasCurrent || !this.currentTicker) {
        this.currentTicker = tickers[0].symbol || tickers[0] || "SPY";
      }
  ...
  renderBodyScannerGrid(tickers) {
    ...
    tickers.forEach((ticker) => {
      ...
      item.addEventListener("click", () =>
        this.toggleBodyTicker(ticker.symbol),
      );
  ...
  toggleBodyTicker(symbol) {
    ...
    const bodyItem = document.querySelector(
      `#body-ticker-grid .scanner-body-item[data-symbol="${symbol}"]`,
    );
    if (bodyItem) {
      bodyItem.classList.toggle("selected", isSelected);
      bodyItem.classList.toggle("dimmed", !isSelected);
      bodyItem.querySelector(".scan-status").textContent = isSelected
        ? "Scanning"
        : "Skip";
    }

    const scannerItem = document.querySelector(
      `.scanner-ticker-item[data-symbol="${symbol}"]`,
    );
    if (scannerItem) {
    ...
    const modalItem = document.querySelector(
      `#modal-ticker-list .scanner-modal-item[data-symbol="${symbol}"]`,
    );
    if (modalItem) {
  ...
  bodySelectAll() {
    ...
    document
      .querySelectorAll("#body-ticker-grid .scanner-body-item")
      .forEach((item) => {
  ...
  async runBodyScan() {
    ...
    const bullishOnly =
      document.getElementById("body-filter-bullish")?.checked || false;
    const bearishOnly =
      document.getElementById("body-filter-bearish")?.checked || false;
    const highConfidence =
      document.getElementById("body-filter-high")?.checked || false;
    ...
    try {
      ...
      if (highConfidence) {
        results = results.filter(
          (r) => (r.trade_score || r.confidence || 0) >= 90,
        );
      }
  ...
  displayScannerResults(results, selectedCount = 0) {
    ...
    if (!results || results.length === 0) {
      list.innerHTML =
        headerHtml +
        '<div class="text-muted text-center py-3">No results to display</div>';
    ...
    results.forEach((result) => {
      const isBullish =
        result.direction === "BULLISH" || result.recommendation === "CALLS";
      const isBearish =
        result.direction === "BEARISH" || result.recommendation === "PUTS";
      ...
      item.style.background = isBullish
        ? "rgba(34, 197, 94, 0.15)"
        : isBearish
          ? "rgba(239, 68, 68, 0.15)"
          : "rgba(234, 179, 8, 0.15)";
  ...
  updateAllScanCounts() {
    ...
    const selected = Object.values(this.scannerTickerSelection).filter(
      (v) => v,
    ).length;
  ...
  renderScannerTickerList(tickers) {
    ...
    tickers.forEach((ticker) => {
      ...
      item.addEventListener("click", () =>
        this.toggleScannerTicker(ticker.symbol),
      );
      container.appendChild(item);
    });
  ...
  saveScannerSelection() {
    localStorage.setItem(
      "scannerTickerSelection",
      JSON.stringify(this.scannerTickerSelection),
    );
  }
  ...
  toggleModalTicker(symbol) {
    ...
    if (modalItem) {
      modalItem.classList.toggle(
        "selected",
        this.scannerTickerSelection[symbol],
      );
      modalItem.classList.toggle(
        "dimmed",
        !this.scannerTickerSelection[symbol],
  ...
  modalSelectAll() {
    ...
    document
      .querySelectorAll("#modal-ticker-list .scanner-modal-item")
      .forEach((item) => {
        item.classList.add("selected");
        item.classList.remove("dimmed");
      });
  ...
  updateNavBadge() {
    ...
    if (badge) {
      ...
      badge.className =
        "badge ms-1 " +
        (selected === 0 ? "bg-danger text-white" : "bg-light text-dark");
    }
  }
  ...
  async runModalScan() {
    ...
    const filters = {
      bullish_only:
        document.getElementById("modal-filter-bullish")?.checked || false,
      bearish_only:
        document.getElementById("modal-filter-bearish")?.checked || false,
    ...
    try {
      ...
      if (summary && data.summary) {
        document.getElementById("summary-analyzed").textContent =
          `${data.summary.successful}/${data.summary.total_analyzed}`;
        document.getElementById("summary-bullish").textContent =
          data.summary.bullish_setups;
        document.getElementById("summary-bearish").textContent =
          data.summary.bearish_setups;
        document.getElementById("summary-high-conf").textContent =
          data.summary.high_confidence;
      ...
      if (results) {
        if (!data.results || data.results.length === 0) {
          results.innerHTML =
            '<div class="text-center text-muted py-3">No stocks matched your filters</div>';
        } else {
          results.innerHTML = data.results
            .map((r) => this.renderScanResult(r))
            .join("");
        }
      }
    } catch (error) {
  ...
  async loadSignals() {
    try {
      ...
      if (signals.length === 0) {
        feed.innerHTML =
          '<div class="list-group-item bg-dark text-muted text-center py-3">No signals yet</div>';
        return;
      }

      signals
        .slice(0, 10)
        .forEach((signal) => this.addSignalToFeed(signal, false));
    } catch (error) {
  ...
  bindEvents() {
    ...
    document.querySelectorAll(".timeframe-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        document.getElementById("chart-timeframe-label").textContent =
    ...
    if (toggleAdvanced) {
      toggleAdvanced.addEventListener("click", () => {
        ...
        if (section)
          section.style.display = this.advancedVisible ? "block" : "none";
        toggleAdvanced.classList.toggle("btn-primary", this.advancedVisible);
        toggleAdvanced.classList.toggle(
    ...
    if (addTickerSubmit)
      addTickerSubmit.addEventListener("click", () => this.addTicker());
    ...
    if (audioVolumeSlider) {
      audioVolumeSlider.addEventListener("input", (e) => {
        ...
        document.getElementById("volume-level-display").textContent =
          e.target.value + "%";
      });
    }
    ...
    if (testBuySound)
      testBuySound.addEventListener("click", () => this.playBuyAlert());

    const testSellSound = document.getElementById("test-sell-sound");
    if (testSellSound)
      testSellSound.addEventListener("click", () => this.playSellAlert());
    ...
    if (paperBuyBtn)
      paperBuyBtn.addEventListener("click", () => this.openPaperTrade("long"));
    if (paperSellBtn)
      paperSellBtn.addEventListener("click", () =>
        this.openPaperTrade("short"),
      );

    const paperExecute = document.getElementById("paper-execute");
    if (paperExecute)
      paperExecute.addEventListener("click", () => this.executePaperTrade());
    ...
    if (selectAllBtn)
      selectAllBtn.addEventListener("click", () => this.scannerSelectAll());

    const deselectAllBtn = document.getElementById("scanner-deselect-all");
    if (deselectAllBtn)
      deselectAllBtn.addEventListener("click", () => this.scannerDeselectAll());

    const openScannerModal = document.getElementById("open-scanner-modal");
    if (openScannerModal)
      openScannerModal.addEventListener("click", () => this.openScannerModal());

    const modalSelectAll = document.getElementById("modal-select-all");
    if (modalSelectAll)
      modalSelectAll.addEventListener("click", () => this.modalSelectAll());

    const modalDeselectAll = document.getElementById("modal-deselect-all");
    if (modalDeselectAll)
      modalDeselectAll.addEventListener("click", () => this.modalDeselectAll());

    const modalScanBtn = document.getElementById("modal-scan-btn");
    if (modalScanBtn)
      modalScanBtn.addEventListener("click", () => this.runModalScan());

    const bodySelectAll = document.getElementById("body-select-all");
    if (bodySelectAll)
      bodySelectAll.addEventListener("click", () => this.bodySelectAll());

    const bodyDeselectAll = document.getElementById("body-deselect-all");
    if (bodyDeselectAll)
      bodyDeselectAll.addEventListener("click", () => this.bodyDeselectAll());

    const bodyScanBtn = document.getElementById("body-scan-btn");
    if (bodyScanBtn)
      bodyScanBtn.addEventListener("click", () => this.runBodyScan());

    const prevTickerBtn = document.getElementById("prev-ticker-btn");
    if (prevTickerBtn)
      prevTickerBtn.addEventListener("click", () => this.navigateTicker(-1));

    const nextTickerBtn = document.getElementById("next-ticker-btn");
    if (nextTickerBtn)
      nextTickerBtn.addEventListener("click", () => this.navigateTicker(1));

    const exportScanBtn = document.getElementById("export-scan-btn");
    if (exportScanBtn)
      exportScanBtn.addEventListener("click", () => this.exportScanResults());

    const refreshNewsBtn = document.getElementById("refresh-news-btn");
    if (refreshNewsBtn)
      refreshNewsBtn.addEventListener("click", () => this.loadNewsData());
  }
  ...
  navigateTicker(direction) {
    ...
    const currentIndex = options.findIndex(
      (opt) => opt.value === this.currentTicker,
    );
  ...
  updateDataStaleness() {
    ...
    if (lastRefreshEl && this.lastRefreshTimestamp) {
      const elapsed = Math.floor(
        (Date.now() - this.lastRefreshTimestamp) / 1000,
      );
      if (elapsed > 15) {
  ...
  async updateMarketStatus() {
    ...
    try {
      ...
      if (badge) {
        ...
        badge.className =
          "badge " + (status.is_market_open ? "bg-success" : "bg-secondary");
      }
      const sessionName = document.getElementById("session-name");
      if (sessionName)
        sessionName.textContent = status.current_session.replace(/_/g, " ");
      ...
      if (textEl)
        textEl.textContent =
          "Data loaded. Market status will update when server responds.";
  ...
  updateSessionBanner(status) {
    ...
    if (!status || session == null) {
      ...
      if (session === "PRE_MARKET") {
        ...
        badgeClass = "bg-info";
        checklist =
          "Pre-market analysis active. Confirm signals after market opens.";
        ...
        checklist =
          "Use this time to review today's signals and prepare for tomorrow.";
        ...
        badgeClass = "bg-secondary";
        checklist =
          "Market is closed. Signals will activate when trading resumes.";
        bannerBg = "rgba(100, 100, 100, 0.15)";
      }
    }
  ...
  async refreshData() {
    ...
    Promise.allSettled(
      critical.map((fn) =>
        fn().catch((e) => console.warn("Critical Refresh:", e)),
      ),
    ).then(() => {
      this.clearLoadingState();

      Promise.allSettled(
        secondary.map((fn) => fn().catch((e) => console.warn(e))),
      ).then(() => {
        this.updateWinRate();
        if (this.advancedVisible) this.loadAdvancedData();
      });
      ...
      if (refreshText) {
        const selected = Object.keys(this.scannerTickerSelection || {}).filter(
          (k) => this.scannerTickerSelection[k],
        ).length;
  ...
  clearLoadingState() {
    ...
    if (
      ...
      (currentPriceEl.textContent === "$--" ||
        currentPriceEl.textContent.includes("--"))
    ) {
      currentPriceEl.innerHTML =
        '<span class="text-muted">Click Refresh for live price</span>';
    }
  ...
  async loadMultiTimeframeAnalysis() {
    try {
      const response = await fetch(
        `/api/multi-timeframe/${this.currentTicker}?_t=${Date.now()}`,
      );
      const data = await response.json();
  ...
  async loadScalpingLevels() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/scalping-levels/${this.currentTicker}?_t=${Date.now()}`,
      );
      const data = await response.json();
      if (loadingEl) loadingEl.style.display = "none";
      if (data.error) {
        if (bestRangeEl)
          bestRangeEl.innerHTML = `<span class="text-warning small">${data.error}</span>`;
      ...
      if (fibLevelsEl && Object.keys(levels).length) {
        const parts = ["23.6", "38.2", "50", "61.8", "78.6"]
      ...
      tfOrder.forEach((tf) => {
        ...
        const zone = fib.zone || "—";
        const ret =
          fib.retracement_pct != null ? fib.retracement_pct + "%" : "—";
      ...
      if (timeframesEl)
        timeframesEl.innerHTML =
          tfHtml || '<span class="text-muted small">No timeframe data</span>';
    } catch (error) {
  ...
  updateEntryTimingNote(confluence) {
    ...
    if (mainSignal === "PREPARE") {
      if (isBullish) {
        noteText =
          '<i class="bi bi-info-circle"></i> Higher timeframe bias bullish — entry timing pending.';
      } else {
        noteText =
          '<i class="bi bi-info-circle"></i> Higher timeframe bias bearish — entry timing pending.';
      }
      noteEl.style.display = "block";
    } else if (mainSignal.includes("BUY") || mainSignal.includes("SELL")) {
      noteText =
        '<i class="bi bi-check-circle text-success"></i> Entry confirmed on primary timeframe.';
      noteEl.style.display = "block";
    } else {
  ...
  async loadEarningsData() {
    try {
      ...
      if (earningsWarning && data.warning) {
        ...
        earningsText.textContent =
          data.message || `Earnings in ${data.days_until} days!`;
  ...
  async loadNewsData() {
    try {
      ...
      if (data.news && data.news.length > 0) {
        ...
        newsList.innerHTML =
          '<div class="list-group-item bg-transparent text-muted border-0">No recent news</div>';
      }
    } catch (error) {
  ...
  async loadOptionsFlowData() {
    try {
      const response = await fetch(
        `/api/options-flow/${this.currentTicker}?_t=${Date.now()}`,
      );
      const data = await response.json();
      ...
      if (ivBadge && data.iv_rank !== undefined) {
        ...
        if (data.iv_rank < 25) {
          ...
          ivBadge.title =
            "IV is LOW - Options are cheap. Good time to buy options!";
          ...
          ivBadge.title =
            "IV is HIGH - Options are expensive! Consider selling instead.";
        }
      }
    } catch (error) {}
  }
  ...
  updateWinRate() {
    ...
    const signals = this.signalHistory.filter(
      (s) => s.ticker === this.currentTicker,
    );
  ...
  async loadTradeRecommendation() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/trade-recommendation/${this.currentTicker}?interval=${this.currentInterval}&_t=${cacheBuster}`,
      );
      const data = await response.json();
      ...
      if (data.error || !data.current_price) {
        ...
        if (currentPriceEl)
          currentPriceEl.innerHTML =
            '<span class="text-muted">Refresh for live price</span>';
      ...
      if (typeof this.loadScalpingLevels === "function")
        this.loadScalpingLevels();
      ...
      if (isNewSignal && data.main_signal !== "WAIT" && this.audioEnabled) {
        if (data.main_signal === "BUY" || data.main_signal === "STRONG BUY") {
          ...
          data.main_signal === "SELL" ||
          data.main_signal === "STRONG SELL"
        ) {
          this.playSellAlert();
          this.sendPushNotification(
            `${data.main_signal}: ${this.currentTicker}`,
  ...
  getSignalBadge(data) {
    ...
    const isBuySell =
      signalState.includes("BUY") || signalState.includes("SELL");
  ...
  updateDebugPanel(data) {
    ...
    setEl(
      "debug-confidence",
      data.confidence_pct ? `${data.confidence_pct.toFixed(1)}%` : "--",
    );
    setEl(
      "debug-volume",
      data.indicators?.volume?.spike_ratio
        ? `${data.indicators.volume.spike_ratio.toFixed(2)}x`
        : "--",
    );
    setEl(
      "debug-signal",
      `${data.main_signal} (raw: ${data.raw_signal || "--"})`,
    );
  ...
  updateConfidenceFactors(data) {
    ...
    const tfPass = data.timeframe_confluence
      ? data.timeframe_confluence >= 3
      : null;
  ...
  updateTrafficLight(signal) {
    ...
    if (signal === "BUY" || signal === "STRONG BUY") {
      document
        .querySelector(".traffic-light .light.green")
        ?.classList.add("active");
    } else if (signal === "SELL" || signal === "STRONG SELL") {
      document
        .querySelector(".traffic-light .light.red")
        ?.classList.add("active");
    } else if (signal === "PREPARE") {
      document
        .querySelector(".traffic-light .light.yellow")
        ?.classList.add("active");
    } else {
      document
        .querySelector(".traffic-light .light.yellow")
  ...
  updateMainSignalPanel(data) {
    ...
    if (edgeEl && data.edge_direction) {
      ...
      if (data.edge_direction === "FLAT") {
        ...
        const cls =
          data.edge_direction === "CALL" ? "text-success" : "text-danger";
        const pct =
          data.edge_pct != null && !isNaN(data.edge_pct) ? data.edge_pct : "";
    ...
    if (confidence) {
      const tierClass =
        data.confidence_tier === "high"
          ? "bg-success fw-bold"
          : data.confidence_tier === "normal"
            ? "bg-warning text-dark"
            : "bg-secondary opacity-75";
      const confValue =
        data.confidence_pct != null && !isNaN(data.confidence_pct)
    ...
    if (
      ...
      reasonsList.innerHTML = data.reasons
        .map((r) => {
          const icon =
            r.includes("bullish") || r.includes("above") || r.includes("SPIKE")
    ...
    if (
      data.main_signal === "STRONG BUY" &&
      this.lastSignal?.type !== "STRONG BUY"
    ) {
      ...
      data.main_signal !== "WAIT" &&
      data.main_signal !== "WATCH" &&
      this.lastSignal?.type !== data.main_signal
    ) {
      this.logSignal(
        this.currentTicker,
        data.main_signal,
  ...
  updateRecommendationPanels(data) {
    ...
    if (showPanel) {
      if (actionPanel) {
        ...
        let borderColor = data.main_signal.includes("BUY")
          ? "success"
          : data.main_signal.includes("SELL")
            ? "danger"
            : data.main_signal === "PREPARE"
              ? "warning"
              : "secondary";
        let headerColor = data.main_signal.includes("BUY")
          ? "bg-success"
          : data.main_signal.includes("SELL")
            ? "bg-danger"
            : data.main_signal === "PREPARE"
      ...
      if (tradeRec) {
        let textColor = data.main_signal.includes("BUY")
          ? "success"
          : data.main_signal.includes("SELL")
            ? "danger"
            : data.main_signal === "PREPARE"
        ...
        let displayText =
          data.main_signal === "PREPARE"
            ? `PREPARE ${this.currentTicker} ${optionDisplay}`
            : data.main_signal === "WATCH"
              ? `WATCHING ${this.currentTicker}`
      ...
      if (hardStopEl) {
        const hardStopValue =
          data.hard_stop != null && !isNaN(data.hard_stop)
            ? data.hard_stop
            : data.stop;
      ...
      if (optionSuggestion)
        optionSuggestion.textContent = `${this.currentTicker} $${data.strike} ${data.option_type}`;

      document.getElementById("suggested-strike")?.textContent &&
        (document.getElementById("suggested-strike").textContent =
          "$" + data.strike);
      document.getElementById("suggested-expiry")?.textContent &&
        (document.getElementById("suggested-expiry").textContent = data.expiry);
      document.getElementById("position-size")?.textContent &&
        (document.getElementById("position-size").textContent =
          data.position_contracts + " contracts");
      document.getElementById("max-risk")?.textContent &&
        (document.getElementById("max-risk").textContent =
          "$" + data.max_risk.toFixed(0));
      ...
      if (whyPanel) {
        ...
        if (reasonsList && data.reasons) {
          reasonsList.innerHTML = data.reasons
            .map((r) => `<li class="text-light">${r}</li>`)
            .join("");
        }
      }
    } else {
  ...
  updateIndicatorsSummary(indicators) {
    ...
    if (macdValue && indicators.macd) {
      ...
      const macdClass = macdType.includes("BULLISH")
        ? "bg-success"
        : macdType.includes("BEARISH")
          ? "bg-danger"
          : "bg-secondary";
      if (macdSignal) {
    ...
    if (volumeValue && indicators.volume) {
      volumeValue.textContent =
        (indicators.volume.spike_ratio || 1).toFixed(1) + "x";
      if (volumeSignal) {
        volumeSignal.textContent = indicators.volume.spike ? "SPIKE" : "Normal";
        volumeSignal.className =
          "badge " +
          (indicators.volume.spike ? "bg-warning" : "bg-secondary") +
          " small";
      }
    }
    ...
    if (vwapValue && indicators.vwap) {
      ...
      if (vwapSignal) {
        ...
        vwapSignal.className =
          "badge " +
          (indicators.vwap.above_vwap ? "bg-success" : "bg-danger") +
          " small";
      }
    }
    ...
    if (indicators.ema) {
      ...
      const formatEma = (val, label) => {
        ...
        const color =
          price > val ? "#22C55E" : price < val ? "#EF4444" : "#9CA3AF";
      ...
      if (
        ...
        indicators.ema.crossovers.length > 0
      ) {
        crossoverAlert.style.display = "block";
        crossoverAlert.textContent = indicators.ema.crossovers
          .map((c) => `${c.type} (${c.pair})`)
          .join(", ");
    ...
    let bullish = 0,
      bearish = 0;
    if (indicators.rsi?.value < 30) bullish++;
    else if (indicators.rsi?.value > 70) bearish++;
    if (indicators.macd?.signal_type?.includes("BULLISH")) bullish++;
    else if (indicators.macd?.signal_type?.includes("BEARISH")) bearish++;
    if (indicators.vwap?.above_vwap) bullish++;
    else bearish++;
    if (indicators.trend?.direction === "BULLISH") bullish++;
    else if (indicators.trend?.direction === "BEARISH") bearish++;
    ...
    if (stripPrice && this.lastPrice)
      stripPrice.textContent = "$" + this.lastPrice.toFixed(2);
    if (stripEma13 && indicators.ema?.ema_13)
      stripEma13.textContent = indicators.ema.ema_13.toFixed(1);
    if (stripEma48 && indicators.ema?.ema_48)
      stripEma48.textContent = indicators.ema.ema_48.toFixed(1);
  ...
  updatePriceDisplay(data) {
    ...
    if (currentPrice) {
      ...
      if (data.session === "afterhours") {
        sessionLabel =
          '<span class="badge bg-secondary ms-2" style="font-size: 0.5em;" title="After-hours data may be delayed 10-20 min">AH DELAYED</span>';
      } else if (data.session === "premarket") {
        sessionLabel =
          '<span class="badge bg-secondary ms-2" style="font-size: 0.5em;" title="Pre-market data may be delayed 10-20 min">PM DELAYED</span>';
      }
      currentPrice.innerHTML =
        "$" + data.current_price.toFixed(2) + sessionLabel;
    }
  ...
  updateKeyLevels(sr, currentPrice) {
    ...
    document.getElementById("resistance-level")?.textContent &&
      (document.getElementById("resistance-level").textContent =
        "$" + (sr.resistance || 0).toFixed(2));
    document.getElementById("support-level")?.textContent &&
      (document.getElementById("support-level").textContent =
        "$" + (sr.support || 0).toFixed(2));
    document.getElementById("sr-current-price")?.textContent &&
      (document.getElementById("sr-current-price").textContent =
    ...
    if (positionBar && sr.support && sr.resistance) {
      ...
      const position =
        range > 0 ? ((currentPrice - sr.support) / range) * 100 : 50;
  ...
  async loadChartData() {
    ...
    try {
      const cacheBuster = Date.now();
      const response = await fetch(
        `/api/market-data/${this.currentTicker}?period=${this.currentPeriod}&interval=${this.currentInterval}&_t=${cacheBuster}`,
      );
      const data = await response.json();
      ...
      const indicatorsRes = await fetch(
        `/api/indicators/${this.currentTicker}?period=${this.currentPeriod}&interval=${this.currentInterval}&_t=${cacheBuster}`,
      );
  ...
  renderLineChart(data, indicators) {
    ...
    if (indicators.ema) {
      ...
      this.chart.data.datasets[1].data = this.padArray(
        indicators.ema.ema_13_series || [],
        len,
      );
      this.chart.data.datasets[2].data = this.padArray(
        indicators.ema.ema_48_series || [],
        len,
      );
      this.chart.data.datasets[3].data = this.padArray(
        indicators.ema.ema_200_series || [],
        len,
      );
    ...
    const support =
      indicators.support_resistance?.support ||
      (data.closes ? Math.min(...data.closes) * 0.998 : 0);
    const resistance =
      indicators.support_resistance?.resistance ||
      (data.closes ? Math.max(...data.closes) * 1.002 : 0);
    this.chart.data.datasets[4].data = new Array(labels.length).fill(support);
    this.chart.data.datasets[5].data = new Array(labels.length).fill(
      resistance,
    );
  ...
  renderCandlestickChart(data, indicators) {
    ...
    if (indicators.ema) {
      const emaData = (series) =>
        timestamps.map((t, i) => ({
          x: new Date(t).getTime(),
          y: series[i] || null,
        }));

      const ema13Sliced = (indicators.ema.ema_13_series || []).slice(
        sliceStart,
      );
      const ema48Sliced = (indicators.ema.ema_48_series || []).slice(
        sliceStart,
      );
      const ema200Sliced = (indicators.ema.ema_200_series || []).slice(
        sliceStart,
      );
      this.chart.data.datasets[1].data = emaData(
        this.padArray(ema13Sliced, timestamps.length),
      );
      this.chart.data.datasets[2].data = emaData(
        this.padArray(ema48Sliced, timestamps.length),
      );
      this.chart.data.datasets[3].data = emaData(
        this.padArray(ema200Sliced, timestamps.length),
      );
    }
  ...
  renderOHLCBars(data, indicators) {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (ctx) =>
                  `O: ${data.opens[ctx.dataIndex]?.toFixed(2)} H: ${data.highs[ctx.dataIndex]?.toFixed(2)} L: ${data.lows[ctx.dataIndex]?.toFixed(2)} C: ${data.closes[ctx.dataIndex]?.toFixed(2)}`,
              },
            },
          },
          scales: {
    ...
    for (let i = 0; i < closes.length; i++) {
      ...
      bgColors.push(
        bullish ? "rgba(0, 230, 118, 0.7)" : "rgba(255, 82, 82, 0.7)",
      );
  ...
  updateMultiTimeframeDisplay(data) {
    ...
    if (confluenceEl && data.confluence_score !== undefined) {
      ...
      confluenceEl.className =
        data.confluence_score > 0
  ...
  updateVixDisplay(vix) {
    ...
    document.getElementById("vix-trend")?.textContent &&
      (document.getElementById("vix-trend").textContent = vix.trend || "--");
  ...
  updateOptionsFlowDisplay(options) {
    ...
    document.getElementById("pc-ratio")?.textContent &&
      (document.getElementById("pc-ratio").textContent =
        options.put_call_ratio?.toFixed(2) || "--");
    document.getElementById("total-call-vol")?.textContent &&
      (document.getElementById("total-call-vol").textContent =
        this.formatNumber(options.total_call_volume || 0));
    document.getElementById("total-put-vol")?.textContent &&
      (document.getElementById("total-put-vol").textContent = this.formatNumber(
        options.total_put_volume || 0,
      ));
    ...
    if (hotCalls)
      hotCalls.style.display = options.indicators?.hot_calls
        ? "inline-block"
        : "none";
    if (unusualPuts)
      unusualPuts.style.display = options.indicators?.unusual_puts
        ? "inline-block"
        : "none";
    if (blockTrades)
      blockTrades.style.display = options.indicators?.block_trades
        ? "inline-block"
        : "none";
    if (sweeps)
      sweeps.style.display = options.indicators?.sweeps
        ? "inline-block"
        : "none";
  }
  ...
  updateInstitutionalDisplay(institutional) {
    ...
    if (activity) {
      ...
      activity.className =
        "h5 mb-0 " +
        (institutional.activity_level === "HIGH"
          ? "text-warning"
          : "text-secondary");
    }

    document.getElementById("inst-buy-spikes")?.textContent &&
      (document.getElementById("inst-buy-spikes").textContent =
        institutional.bullish_spikes || 0);
    document.getElementById("inst-sell-spikes")?.textContent &&
      (document.getElementById("inst-sell-spikes").textContent =
        institutional.bearish_spikes || 0);
  }
  ...
  addSignalToFeed(signal, prepend = true) {
    ...
    const typeClass = signal.signal_type?.includes("BUY")
      ? "text-success"
  ...
  async addTicker() {
    ...
    try {
      ...
      added.forEach((t) => {
        this.scannerTickerSelection[t.symbol] = true;
      });
      this.saveScannerSelection();
      ...
      if (added.length) {
        this.showTickerToast(
          `Added: ${added.map((t) => t.symbol || t).join(", ")}. Loading data…`,
          "success",
        );
      ...
      if (errors.length)
        this.showTickerToast(errors.slice(0, 3).join("; "), "warning");

      const modal = bootstrap.Modal.getInstance(
        document.getElementById("addTickerModal"),
      );
      if (modal) modal.hide();
    } catch (error) {
  ...
  async saveSettings() {
    ...
    localStorage.setItem(
      "indicatorToggles",
      JSON.stringify(this.indicatorToggles),
    );

    const settings = {
      rsi_period: parseInt(document.getElementById("rsi-period")?.value) || 14,
      rsi_oversold:
        parseInt(document.getElementById("rsi-oversold")?.value) || 30,
      rsi_overbought:
        parseInt(document.getElementById("rsi-overbought")?.value) || 70,
      volume_spike_threshold:
        parseFloat(document.getElementById("volume-threshold")?.value) || 2.0,
      audio_enabled:
        document.getElementById("audio-notifications")?.checked ?? true,
      audio_volume:
        parseInt(document.getElementById("audio-volume")?.value) || 50,
    };
  ...
  updateAudioToggle() {
    ...
    if (toggle) {
      toggle.innerHTML = this.audioEnabled
        ? '<i class="bi bi-volume-up"></i>'
        : '<i class="bi bi-volume-mute"></i>';
      toggle.className =
        "btn btn-sm " +
        (this.audioEnabled ? "btn-outline-success" : "btn-outline-secondary");
    }
  }
    ...
    try {
      ...
      if (positions && data.positions) {
        if (data.positions.length === 0) {
          ...
          positions.innerHTML = data.positions
            .map(
              (p) => `
  ...
  openPaperTrade(side) {
    ...
    if (title)
      title.textContent = side === "long" ? "Buy (Long)" : "Sell (Short)";
    ...
    if (execute) {
      execute.className =
        "btn btn-sm " + (side === "long" ? "btn-success" : "btn-danger");
      execute.textContent = side === "long" ? "Buy" : "Sell";
    }

    const modal = new bootstrap.Modal(
      document.getElementById("paperTradeModal"),
    );
    modal.show();
  }
  ...
  async executePaperTrade() {
    ...
    const quantity =
      parseInt(document.getElementById("paper-quantity")?.value) || 10;
  ...
  async scanTop10() {
    ...
    try {
      ...
      if (results)
        results.innerHTML =
          '<div class="text-center text-danger py-3">Error scanning markets</div>';
    } finally {
      if (btn) btn.disabled = false;
  ...
  async scanCheapOptions() {
    ...
    resultsEl.innerHTML =
      '<div class="text-center text-muted small py-2"><i class="bi bi-hourglass-split"></i> Scanning 50 liquid stocks...</div>';
